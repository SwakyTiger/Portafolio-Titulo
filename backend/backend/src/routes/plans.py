...
def find_all_plans():
    return plansEntity(conn.transcriptor.planes.find())

@plans.post('/plans', tags=["plans"])
def create_plan(plan: Plan):
    existing_plan = conn.transcriptor.planes.find_one({"id_plan": plan.id_plan})
    if existing_plan:
        raise HTTPException(
    ...
    new_plan = dict(plan)
    id = conn.transcriptor.planes.insert_one(new_plan).inserted_id

    plan = conn.transcriptor.planes.find_one({"_id": id})
    return planEntity(plan)

@plans.get('/plans/{id}', tags=["plans"])
def find_plan(id_plan: int ):
    return planEntity(conn.transcriptor.planes.find_one({"id_plan": id_plan}))

@plans.put('/plans/{id}', response_model=Plan, tags=["plans"])
def update_plan(id: int, plan: Plan):
    result = conn.transcriptor.planes.find_one_and_update(
        {"id_plan": id},
        {"$set": dict(plan)},
...
def delete_plan(id: int):
    result = conn.transcriptor.planes.find_one_and_delete({"id_plan": id})
    if not result:
        raise HTTPException(
            status_code=status.HTTP_404_NOT_FOUND,
            detail=f"El plan con id {id} no existe."
