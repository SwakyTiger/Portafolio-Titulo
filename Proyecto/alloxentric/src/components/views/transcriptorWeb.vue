    ...
    methods: {
      ...
      async get_user_data() {
        if (keycloak.authenticated) {
          ...
          this.numeroTelefono = token.telefono || '';
          //this.numeroTelefono = `${phonePrefix}${phoneNumber}`.trim();
          
          // Llamar a find_usuario para obtener información completa del usuario
          await this.obtenerInformacionUsuario();
        }
      },
      async obtenerInformacionUsuario() {
        try {
          const response = await axios.get(`http://localhost:8000/bot/${this.numeroTelefono}`);
      ...
      async restarCredito() {
        try {
          await axios.put(`http://localhost:8000/restar-creditos/${this.numeroTelefono}`);
